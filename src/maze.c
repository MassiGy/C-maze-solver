...
void findStartNEndNTarget(maze_t *playground) {
    for (int i = 0; i < playground->row_count; i++)
    {
        for (int j = 0; j < playground->col_count; j++)
        {
            switch (playground->grid[i][j])
            {
            case 2:
                playground->fstEntry[0] = i;
                playground->fstEntry[1] = j;
                break;
            case 3:
                playground->fstEnd[0] = i;
                playground->fstEnd[1] = j;
                break;
            case 4:
                playground->sndEntry[0] = i;
                playground->sndEntry[1] = j;
                break;
            case 5:
                playground->sndEnd[0] = i;
                playground->sndEnd[1] = j;
                break;
            case 6:
                playground->target[0] = i;
                playground->target[1] = j;
                break;
            }
        }
        ...
        if (is_up_possible)
        {
            ...
            if (current_checkpoint->limited_threads)
            {
                ...
                if (thereis_free_thread)
                {
                    ...
                    /* also push the thread next move to its current_track_record */
            ...
            {
                /*  push the thread next move to its current_track_record */
                up_thread_checkpoint.current_track_record = push_list(up_thread_checkpoint.current_track_record, current_checkpoint->last_pos - (maze.col_count));
                pthread_create(&up_thread, NULL, &solveMaze_threaded, &up_thread_checkpoint);
            }
        }
