...
int main(void)
{
     ...
     * 
     * NOTE: Do not set a small limit for the threads count, since this can cause an error. So if you are getting false results when using limited threads mode,
     * try increasing the threads_limit_count.
     */
    ...
    free(fst_checkpoint.lock);
    pthread_mutex_destroy(snd_checkpoint.lock);
    free(snd_checkpoint.lock);
